 ...
 * Exceptions are logged together with stack traces. Default priority for
 * method entrance/exit message is DEBUG, for exceptions - ERROR; but both can
 * be overridden.
 */
public final class Log4jUtility {
    /**
     ...
     *
     * @param logger      the logger to be used (null if logging is not
     ...
     * @param paramNames  the names of input parameters (null of method doesn't
     *                    accept any parameters)
     */
    public static void logEntrance(final Logger logger,
     ...
     * When this method is used, method execution time is not logged and DEBUG
     * priority is used.
     *
     * @param value     the value returned from the method (should contain 1
     *                  element with the returned value, or should be null if
     ...
     *                  (e.g. className#methodName)
     */
    public static void logExit(final Logger logger,
                               final String signature,
                               final Object[] value) {
     ...
     * When this method is used, DEBUG priority is used.
     *
     * @param value             the value returned from the method (should
     *                          contain 1 element with the returned value, or
     *                          should be null if the method returns void)
     * @param logger            the logger to be used (null if logging is not
     *                          required to be performed)
     ...
     *                          method (e.g. className#methodName)
     */
    public static void logExit(final Logger logger, final String signature,
                               final Object[] value,
                               final Date entranceTimestamp,
                               final Priority priority) {
        if (logger == null) {
        ...
        logger.log(priority, LoggingUtilityHelper
                .getMethodExitMessage(signature, entranceTimestamp));
        if (value != null) {
            logger.log(priority, LoggingUtilityHelper
                    .getOutputValueMessage(value[0]));
        }
    }
     ...
     * @param logger    the logger to be used (null if logging is not required
     *                  to be performed)
     * @param signature the signature that uniquely identifies the method (e.g.
     *                  className#methodName) where the exception is logged
     * @return the logged exception
     */
    public static <T extends Throwable> T logException(final Logger logger,
                                                       final String signature,
                                                       final T exception) {
    ...
    public static <T extends Throwable> T logException(final Logger logger,
        ...
        logger.log(priority, LoggingUtilityHelper
                .getExceptionMessage(signature, exception));
        return exception;
    }
}
